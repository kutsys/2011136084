...
int main(int argc, char **argv){
	...
	if(eident[EI_CLASS] == ELFCLASS32) {
		...
		if(opt & OPT_ELF_HEADER)
			elfreaderHeader32(elfEhdr);
		if(opt & OPT_SEC_HEADER)
			elfreaderSectionHeader32(fp, elfEhdr);
		if(opt & OPT_PRO_HEADER)
			elfreaderProgramHeader32(fp, elfEhdr);
	}
	else if(eident[EI_CLASS] == ELFCLASS64) {
		Elf64_Ehdr elfEhdr;
		fread(&elfEhdr, 1, sizeof(elfEhdr), fp);
		if(opt & OPT_ELF_HEADER)
			elfreaderHeader64(elfEhdr);
		if(opt & OPT_SEC_HEADER)
			elfreaderSectionHeader64(fp, elfEhdr);
		if(opt & OPT_PRO_HEADER)
			elfreaderProgramHeader64(fp, elfEhdr);
	}
	else {
...
void elfreaderProgramHeader32(FILE* fp, Elf32_Ehdr elfEhdr) {
	...
	for(int i=0; i<elfEhdr.e_phnum; i++){
